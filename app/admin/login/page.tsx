...
export default function AdminLoginPage() {const { t } = useTranslation();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (firebaseError.code === "auth/invalid-credential") {
        ...
        errorMessage = t("너무 많은 로그인 시도가 있었습니다. 잠시 후 다시 시도해주세요.");

      } else if (firebaseError.code === "auth/network-request-failed") {
        errorMessage = t("네트워크 오류가 발생했습니다. 인터넷 연결을 확인해주세요.");

      } else if (
      firebaseError.code === "auth/configuration-not-found" ||
      firebaseError.message?.includes("auth/invalid-api-key"))
      {
        errorMessage = t("Firebase Authentication이 올바르게 설정되지 않았습니다. Firebase Console에서 Authentication을 활성화해주세요.");
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 py-16">
      <div className="max-w-md w-full">
        ...
        {/* Header */}
        <h1 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-purple-500 to-pink-600 mb-2 text-center">{t("관리자 로그인")}

        </h1>
        <p className="text-slate-400 text-center mb-8">{t("Showcase 관리 대시보드에 접근하려면 로그인하세요")}
        ...
        <form
          ...
          <div className="mb-6">
            <label className="block text-sm font-semibold text-slate-300 mb-2">{t("📧 이메일")}
          ...
          <div className="mb-6">
            <label className="block text-sm font-semibold text-slate-300 mb-2">{t("🔑 비밀번호")}
        ...
        <div className="mt-8 bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl border border-slate-700 p-6">
          <h3 className="text-sm font-semibold text-white mb-3 flex items-center">
            <span className="mr-2">💡</span>{t("Firebase 설정이 필요하신가요?")}
